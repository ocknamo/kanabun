...
プリミティブ(`lazy`、`<Portal>`、`<Dynamic>`、`<Head>` / `<Title>`)、ルーター
...
エントリ dev サーバーを利用)。SSR の例([`examples/ssr/`](examples/ssr/))はサーバー
として起動します:`bun examples/ssr/server.tsx`。
...
| 型 | `Accessor`, `Signal`, `SignalOptions`, `Disposer`, `Context`, `Props`, `JSXChild`, `JSX`, `EventHandler`, `HTMLAttributes`, `ShowProps`, `ForProps`, `ErrorBoundaryProps`, `RenderToStringResult`, `Resource`, `SuspenseProps`, `LazyModule`, `PortalProps`, `DynamicProps`, `HeadProps`, `TitleProps` |
...
`<Dynamic>`、`<Head>` / `<Title>`)を追加しました。残り ── アイランド + 作成支援ツール
(`kanabun lint`、dev オーバーレイ)、状態保持 HMR ── と未決の設計判断は
