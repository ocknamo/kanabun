...
(`@kanabun/router`)、テストヘルパー(`@kanabun/testing` ── jsdom 無しで
コンポーネントを単体テスト)、そして CLI(`create` / `dev` / `build` / `generate`)を
...
```tsx
// サーバー(server.tsx)── Bun まわりの配管は CLI が持つ: クライアントのバンドル、
// 共有 HTML ドキュメントへのラップ、配信までを `serve` が面倒を見る。
...
(`renderToString` を直接使うこともできます ── サーバーを自前で持ちたいときは
`const { html, head } = renderToString(() => <App />)`。`serve` はその配管の
パッケージ化です: config の形は SSG と共通で、アイランド単位のコード分割をする
`islands` オプションと、HTML シェルを差し替える `document` オーバーライドを持ちます。)
...
kanabun serve ssr.tsx     # SSR サーバー(リクエストごとに描画 + クライアントバンドル)
kanabun preview ssg.tsx   # SSG 出力をビルドして静的配信
```
...
`serve` は SSR 側の対になるコマンドです。config の形は `routes` を除いて SSG と同じ
(`{ render(path), client?, islands?, title?, base?, document? }`)で、ビルド時ではなく
リクエストごとに描画します。クライアントエントリは起動時に一度だけバンドルされページと
並んで配信されます。`islands` マップ(名前 → エントリ)を渡すと `buildIslands` による
アイランド単位のチャンクに切り替わり、ページに存在する島のコードだけがダウンロードされます。
`preview` は `generate` を一時ディレクトリ(または `--outdir`)に走らせて結果を静的配信
します ── 静的デプロイと同じ形の確認用です。どちらもライブラリとしても import できます
(`serve(config, opts)` / `preview(opts)` / 埋め込み用の `createSSRHandler`)。
[`examples/ssr/`](examples/ssr/) と [`examples/islands/`](examples/islands/) 参照。
...
**jsdom/happy-dom 無しで**、任意の JS ランタイム上でコンポーネントを単体テストできます:
kanabun 自身のスイートが走らせているのと同じ小さなインメモリ DOM モックに、
描画・クエリ・イベント・flush のヘルパーを添えてパッケージ化したものです。
*dev* dependency として入れてください。
...
`renderTest` は `document` が無ければモックを自動で設置します(`dispose()` で復元)。
...
| `serve(config, opts?)` | SSR サーバー起動(リクエストごとに描画、client/islands は起動時に一度バンドル)。`{ url, port, stop() }` を返す。 |
| `createSSRHandler(config, opts?)` | SSR の `fetch` ハンドラ(埋め込み/テスト用)。 |
| `preview(opts)` | SSG 出力をビルドして静的配信。`{ url, port, outdir, stop() }` を返す。 |
...
**`@kanabun/testing`**(開発時のみのテストヘルパー。jsdom 不要)
...
packages/
  ...
  cli/         @kanabun/cli — `kanabun` コマンド(Bun 専用層)
    src/        build.ts, generate.ts, serve.ts, preview.ts, islands.ts, document.ts, create.ts, dev.ts, index.ts(argv + ディスパッチ)
    bin/        kanabun.ts
...
テストファイル名は `*.spec.ts`。レンダラは `@kanabun/testing` として出荷される小さな
DOM モックでテストするので、jsdom / happy-dom 依存は不要です ── フレームワーク自身の
スイートがこのパッケージの最初の利用者です。
