...
Bun レイヤーに置き、型チェックで既に頼っている **オンデマンドの TypeScript パーサ**
(`bunx tsc`)を再利用します。これなら vendored もランタイム依存も増えません ── オプトイン
かつ開発時のみの作成支援ツールであって、フレームワークが依存するランタイムコンパイラでは
ありません。
...
- **パーサ。** TypeScript コンパイラ API を `bunx tsc` と同じオンデマンド方式で取得 ── 素の
  `import("typescript")` を Bun の auto-install に任せ、`package.json` には何も足さない
  (「TS はオンデマンド取得・vendored しない/dev 依存は `@types/bun` のみ」と整合)。
  *先に実現性を確認:* マニフェスト記載なしで auto-install の `import("typescript")` が解決
  できるか。だめなら TS を子プロセスとして起動する案にフォールバック。
