...
| 8 | 重量級エコシステム: SSR ストリーミング(`renderToStream`)、リアクティブ store(`createStore`)、`@kanabun/testing` | 🟡 進行中 — **`@kanabun/testing` 完了**。`renderToStream` / `createStore` が残り |
...
- [x] **`@kanabun/testing` ユーティリティ。** 完了 ── アプリ作者が jsdom 無しで
  コンポーネントを単体テストできる第一級のテスト補助パッケージ(`packages/testing/`)。
  リポジトリ内 DOM モック(core から移設 ── `files: ["src"]` が公開境界なので
  パッケージ横断の相対 import は出荷できない)、`renderTest`(モックコンテナへ描画。
  `document` 不在なら自動でモックを設置し `dispose` で復元)、クエリ補助
  (`childByTag` は直下のみ / `queryByTag`+`queryAllByTag`/`queryByClass`+
  `queryAllByClass`+`hasClass` はサブツリー・文書順)、`fireEvent`
  (+ `leftClick` ペイロードを合成する `.click`、`.keyDown`。入力用に
  `typeAndEnter`/`setValue`)、`tick`(macrotask 1 回 ── `onMount`/resource の
  microtask も排出)。依存ゼロ・ランタイム非依存(`bun:test` を import しない ──
  フックの配線は利用者側)・100% カバレッジ: モックはカバレッジ除外のフィクスチャから
  カバー対象の製品コードへ昇格。core + router のスイートが最初の利用者 ── DOM を使う
  全 spec が手書きヘルパーの代わりにこれらを import する。利用者にとっては開発時のみ
  (devDependency)、公開は他パッケージと同様。
  [`decisions.md`](./decisions.md#kanabuntesting-phase-8) を参照。
  **フォローアップ(合意済み・二段階):**
  - [x] **Stage 1 ── spec に残るヘルパーの吸収。** 完了 ── `deferred<T>()`
    (`async.spec.ts` / `lazy.spec.ts` にコピペされていた)、型付きアクセサ
    `docBody()` / `docHead()`(`portal` / `islands` / `head` / `css` spec が
    手書きしていた。document 未設置なら `installDOM` を指す明快なメッセージで
    throw)、`childById` / `queryById`(`control-flow.spec.ts` の `byId`。
    `childByTag` / `queryByTag` と対称の命名)、`styles()` / `ruleFor()`
    (`css.spec.ts` のスコープド CSS 検査。`ruleFor` は assert ではなく
    「ちょうど 1 件でなければ *throw*」に ── パッケージは `bun:test` を
    import しない)、`captureWarnings()`(`dev.spec.ts` の sink コレクタ。
    復帰は `setWarnHandler(null)`。core のプロセス共有 dedupe には手が
    届かない ── docstring 参照)。あわせて `source.spec.ts` の `fakeWindow` /
    `fakeHashWindow` コピペを `router-test-utils.ts` へ集約
    (router ローカルの話で、このパッケージには入れない)。
  - [x] **Stage 2 ── testing-library 風エルゴノミクス(部分採用)。** 完了 ──
    同じルックアップの上に `getBy*` / `queryBy*` の二層 API
    (`getByTag` / `getByClass` / `getById` / `getByText`)。`getBy*` は
    **ちょうど 1 件**を保証する ── miss なら `Unable to find …`、重複なら
    `Found N matches …(expected exactly one)` で、いずれもシリアライズ済み
    ツリーを添えて throw し、testing-library の契約に一致する。`queryBy*` は
    先頭マッチ(または `undefined`)を返し、`queryAllBy*` は全件を返す
    (テキスト/id の `getBy*` も件数を数えられるよう `queryAllByText` /
    `queryAllById` を追加)。要素の**自前テキスト**(直接の子テキストノード連結・
    子要素は除外 ── 最内要素が当たり、先祖は巻き込まれない。文字列同値か
    RegExp、空白正規化なし)にマッチする `getByText` / `queryByText`、
    全サブツリークエリを束縛する `within(root)`、そして `renderTest` が
    コンテナ束縛クエリを返す(`RenderTestResult extends BoundQueries`)。
    意図的に**採用しない**もの: `getByRole`(暗黙 ARIA の
    対応表が必要 ── 意図的に最小なモックと衝突)、`findBy*`/`waitFor`(反応性は
    同期。`await tick()` で足りる)、user-event(モックにはバブリングも
    デフォルトアクションも無い ── `fireEvent` の正直さが肝)。
