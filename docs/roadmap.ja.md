...
- [x] **`@kanabun/testing` ユーティリティ。** 完了 ── アプリ作者が jsdom 無しで
  ...
  **フォローアップ(合意済み・二段階):**
  ...
  - [x] **Stage 2 ── testing-library 風エルゴノミクス(部分採用)。** 完了 ──
    同じルックアップの上に `getBy*`(見つからなければ `Unable to find …` +
    シリアライズ済みツリーで throw)/ `queryBy*`(`undefined` を返す)の
    二層 API(`getByTag` / `getByClass` / `getById` / `getByText`。どちらの層も
    文書順の**先頭**マッチを返す ── testing-library と違い複数マッチでは
    throw しない)、要素の**自前テキスト**(直接の子テキストノード連結・
