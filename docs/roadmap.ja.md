...
- [x] **`@kanabun/testing` ユーティリティ。** 完了 ── アプリ作者が jsdom 無しで
  ...
  **フォローアップ(合意済み・二段階):**
  - [ ] **Stage 1 ── spec に残るヘルパーの吸収。** `deferred<T>()`
    (`async.spec.ts` / `lazy.spec.ts` にコピペ)、型付きアクセサ `docBody()` /
    `docHead()`(`portal` / `islands` / `head` spec が手書き)、`childById` /
    `queryById`(`control-flow.spec.ts` の `byId`)。`styles()` / `ruleFor()`
    (`css.spec.ts`)と `captureWarnings()`(`dev.spec.ts`)も検討。あわせて
    `source.spec.ts` の `fakeWindow` コピペを `router-test-utils.ts` へ集約
    (router ローカルの話で、このパッケージには入れない)。
  - [ ] **Stage 2 ── testing-library 風エルゴノミクス(部分採用)。**
    `getBy*`(見つからなければコンテナのシリアライズ HTML 付きで throw)/
    `queryBy*`(`undefined` を返す)の二層 API、`getByText`、`renderTest` が返す
    コンテナ束縛クエリ。意図的に**採用しない**もの: `getByRole`(暗黙 ARIA の
