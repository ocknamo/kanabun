...
- [ ] **`<Island>` 境界 + レジストリ(コア)。** サーバでは `<div data-island data-props>`
  ラッパへ直列化する境界。クライアントのレジストリ(`name → Component`)+ エントリが
  `[data-island]` を走査し props を復元、それらだけを mount する。`renderToString`(サブツリー)+
  `hydrate`(コンテナ)を再利用 ── 第3の描画経路は作らない。props は JSON 直列化可能なもののみ
  (クロージャ / signal は境界を越えない)。各アイランドは独立した root(context / 所有権ツリーは
  越えない ── 共有状態はモジュールレベルの singleton signal で)。
...
**エコシステムプリミティブ。**
- [x] **`lazy()`。** 完了 ── `lazy(() => import("./X"))` はコンポーネントを動的 `import()` の
  背後に遅延させ、バンドラがそこで code-split する。既出の `<Suspense>`(欠けていた相棒)と統合:
  モジュールの**初回ロード中**だけ最寄りの境界に登録し(resource と同じレジストリ)、解決後は
  モジュールの `default` をその場に描画する。import の Promise はファクトリ側にキャッシュされ、
  複数インスタンス / 事前 `Lazy.preload()` が 1 本の要求を共有する。モジュール解決後に生成された
  インスタンスは同期的に描画。import 失敗時は error をリアクティブグラフへ surface する(外側の
  `<ErrorBoundary>` が捕捉)。`packages/core/src/async.ts` の `<Suspense>` に併設;依存ゼロ・
  カバレッジ 100%・ランタイム非依存。[`decisions.md`](./decisions.md#async--suspense-phase-6) 参照。
