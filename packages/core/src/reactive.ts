...
class ReactiveNode {
  ...
  /** Run and clear teardown callbacks (LIFO), e.g. before a re-run. */
  runCleanups(): void {
  ...
  /** Tear down: run cleanups, detach from sources, become inert. */
  dispose(): void {
...
/**
 * Register a teardown callback for the currently running effect or
 * computed. Runs before the owner's next execution and on disposal.
 * A no-op when called outside a reactive computation.
 */
